...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    error = np.sqrt(sigma2) * np.random.randn(N)  # Replace with code to generate Y
...
def hypothesis_test():
    ...
    fun_message = "Wow! You've found a super rare event!" if p_value <= 0.0001 else None

    # TODO 12: Plot histogram of simulated statistics
    plot3_path = "static/plot3.png"
    # Replace with code to generate and save the plot
    plt.figure(figsize=(8, 6))
    plt.hist(simulated_stats, bins=20, alpha=0.7, color="gray", label="Simulated Statistics")
    plt.axvline(observed_stat, color="red", linestyle="--", label=f"Observed: {observed_stat:.2f}")
    plt.axvline(hypothesized_value, color="blue", linestyle="-", label=f"Hypothesized: {hypothesized_value:.2f}")
...
def confidence_interval():
    ...
    plt.figure(figsize=(10, 5))
    plt.scatter(estimates, [0] * len(estimates), alpha=0.5, color="gray", label="Simulated Estimates")
    plt.axvline(mean_estimate, color="blue", label=f"Mean Estimate: {mean_estimate:.2f}")
    plt.axhline(0, color="black", linewidth=0.5)
    plt.plot([ci_lower, ci_upper], [0, 0], color="green", label=f"{int(confidence_level)}% CI")
    plt.axvline(true_param, color="green", linestyle="--", label=f"True {parameter.capitalize()}: {true_param:.2f}")
